...
namespace ogmpp_planners
{
  namespace prms
  {
    ...
    std::map<std::string,double> ObstacleBasedStarSampling::_fixParameters(
      std::map<std::string, double> parameters)
    {
      ...
      temp_map.insert(std::pair<std::string, double>
        ("obstacle_based_star_sampling_samples", 1000));
      temp_map.insert(std::pair<std::string, double>
        ("obstacle_based_star_sampling_min_neigh_dist", 0.7));
      temp_map.insert(std::pair<std::string, double>
        ("obstacle_based_star_sampling_min_dist_from_wall", 0.3));
      temp_map.insert(std::pair<std::string, double>
        ("obstacle_based_star_sampling_expansion_step", 0.1));
     ...
     */
    ogmpp_graph::Graph ObstacleBasedStarSampling::_createGraph(
        ...
        std::map<std::string, double> parameters)
    {
      ...
      {
        ROS_ERROR_STREAM(
          "ogmpp_obstacle_based_star_sampling: Robot or goal pose is not unoccupied");
        return _g;
      }
      ...
      int samples = p["obstacle_based_star_sampling_samples"];
      double neigh_dist = p["obstacle_based_star_sampling_min_neigh_dist"];
      double min_dist_from_wall = p["obstacle_based_star_sampling_min_dist_from_wall"];
      double expansion_step = p["obstacle_based_star_sampling_expansion_step"];
